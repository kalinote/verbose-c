...
def parse_args():
    ...
    parser.add_argument("--dump", nargs="?", const="all", help="导出执行过程日志，支持模块: parser, tokens, preprocess, ast, opcode, ir, const, label, vm, memory, all；默认 all")
...
def _parse_module_sets(args):
    ...
    allowed_dump_modules = {"parser", "preprocess", "tokens", "ast", "opcode", "ir", "optimize", "const", "label", "vm", "memory", "all"}
