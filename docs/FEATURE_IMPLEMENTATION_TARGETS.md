...
- 目标能力：
  - 【未完成】`O2` 支持 typed AST 或 IR 层局部优化：死分支消除、简单代数化简，以及比 O1 更系统的常量折叠/传播
  - 【未完成】`O3` 支持 IR / CFG 层优化：跨基本块常量传播、死代码消除、局部公共子表达式消除、简单循环优化
  - 【待完善】优化等级与 P0-3 分工需重新确认：`O1` 当前已扩展为解释器后端第一阶段的语义保持优化集合，`O2/O3` 后续应聚焦 typed AST / IR / CFG 层更系统的全局优化
- 当前现状：
...
- 优化主线：**P0-3 `O1`** 是解释器后端第一阶段的语义保持优化；**P2-2 `O2/O3`** 是 typed AST / IR / CFG 层更系统的高级优化；两者验收时均必须保持 VM 行为一致
