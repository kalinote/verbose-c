...
5.  F-P2-1  IR 与控制流图              【未完成】操作码 lowering 到三地址码 IR
...
- 目标能力：
  - 【未完成】定义适合作为多后端输入的三地址码 IR，至少包含基本块、控制流边、临时变量、局部变量、调用、内存读写和类型信息
  - 【未完成】IR 基于现有 `OpcodeGenerator` 生成的栈式操作码序列构建，不从 AST 直接生成；MVP 先覆盖整数、布尔、字符串常量、局部变量、函数、`if`、`while`、`for`
  - 【未完成】在操作码到 IR 的 lowering 阶段恢复显式数据依赖：将 `LOAD_*`、算术/比较、赋值、调用参数、返回值等隐式操作数栈行为转换为 `tN = op a, b` 形式的临时变量定义与使用
  - 【未完成】根据已解析跳转目标切分基本块并构建 CFG；条件跳转、无条件跳转、函数返回和顺序 fallthrough 都应显式表示控制流边
  - 【未完成】IR 保留源码行号或调试映射
  - 【未完成】支持 `--dump ir` 输出文本形式 IR，便于教学和回归对比
- 当前现状：
  - 无 IR 模块；`CompilerPass` 仅有 `TYPE_CHECK`、`GENERATE_CODE`
  - 后端输入目前是栈式字节码和 Python 运行时对象；字节码操作数通过 VM 栈隐式传递，尚无显式 def-use 数据依赖表示
- 验收标准：
  - 简单算术函数、条件分支和循环可从现有操作码序列生成结构正确的三地址码 IR
  - IR 中每个表达式结果、调用结果和局部变量写入都有明确的定义点；每个使用点可追溯到临时变量、局部变量、常量或受支持的内存读
  - IR dump 可对照源码行号，并能明确显示基本块和跳转关系
  - 遇到尚不支持的 opcode、栈高度不匹配或无法恢复数据依赖的控制流合并时，编译器给出明确错误，而不是生成错误 IR
...
- 目标能力：
  ...
  - 【未完成】MVP 支持 P2-1 已覆盖的整数、布尔、字符串常量、局部变量、函数调用、`if`、`while`、`for`，以及最小内置输出调用；全局变量和基础数组可在 C 兼容闭环稳定后纳入
