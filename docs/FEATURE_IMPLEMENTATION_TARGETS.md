...
- 验收标准：
  ...
  - 【已完成】模块 AST 保留原始显式 main 调用标记；`tests/test_execution_semantics.py` 和 `tests/test_native_aot.py` 验证 O1 内联不会误补自动入口，整数和布尔入口保持 O0/O1 一致
...
| 条件                                 | 负责文档        | 行为概要                                                                   |
| ---------------------------------- | ----------- | ---------------------------------------------------------------------- |
| 存在无参 `int main()` / `void main()` / `bool main()` 定义 | C 兼容 P1-8   | 顶层注册 + 按规则自动调用 main；整数返回值为退出码，bool 返回 0/1，void 返回 0 |
...
- 当前现状：
  ...
  - 当前 `COMPILER_REVISION=6`、`.vbb FORMAT_VERSION=2`；修订号 6 使旧源码缓存按修复后的预处理条件与 O1 入口识别规则重新编译。缓存不会扫描编译器 Python 源码或语法文件；开发编译器时需维护修订号，语法更新使用 `-rp` 强制重新生成解析器和编译
...
- 目标能力：
  ...
  - 【已完成】第一版 native 整数、布尔、变量、函数调用及控制流闭环；当前还支持定宽浮点、标准流字符串与有界数组引用，范围见 P2-4 / P2-5。支持无参 int/void/bool main，布尔返回值按 0/1 转为退出码，与 VM 一致
...
- 完成状态：
  - 【已完成】已跑通 `.vbc/.vbb → IR → Machine IR → Windows x64 机器码 → 调试执行 → native 入口返回值` 的 MVP 闭环。
- 目标能力：
  - 【已完成】极小 x64 编码器、Machine IR 指令选择、rel32 修补、保守栈槽分配，以及带源码位置的明确错误报告。
  - 【已完成】整数、布尔、NULL/enum 标量、受限全局标量、分支与循环、用户函数调用、递归、连续栈参数，以及受限 `_exit(int)` / `exit(int)` 退出码传播。
...
- 验收入口：
  ...
  - 专项测试：`tests/test_native_codegen.py`、`tests/test_native_lowering.py`、`tests/test_ir_lowering.py`。
...
- 已完成：
  ...
  - 【已完成】定长局部／全局一维标量数组采用 8 字节长度头和每元素 8 字节存储；全局数组在模块共享帧中初始化，局部数组按调用独立分配。完整数组以 `array_ref:<元素类型>` 单机器字借用引用传参，支持别名、修改、递归转传及栈参数；读写始终检查实际边界。
  - 【已完成】Machine IR 与 map 验证数组来源、布局和引用越界检查；拥有数组的栈帧和调用空间受 4096 字节预算限制。验收入口为 `tests/test_native_arrays.py`、库存示例和分函数排序示例 `tests/grammar/practical_array_sort.vbc`。
- 待完成：
  - 【未完成】数组返回、偏移地址保存、引用写入全局指针、对象元素数组、结构体、一般指针对象存储和逐对象内存回收；扩展前需要单独确定对象布局及生命周期。
  - 【未完成】后续支持可选 GC 安全点及更完整的运行时能力。
- 当前现状：
  - VM 的对象模型、内存管理、GC 和大部分内置函数仍由 Python `verbose_c/vm`、`verbose_c/object` 提供，尚未迁移到 native runtime。
- 验收标准：
  - 多 section、导入和基址重定位等正式 PE/COFF 能力具备结构化校验与可执行测试。
  - native runtime 的最小输出、运行时错误、基础堆分配及其 ABI 可验证，stdout、stderr 和退出码与 VM 约定一致。
  - 正式 AOT 入口可从源码经 IR/native 后端直接生成无需系统 C 编译器的独立 `.exe`，并清晰报告不支持的语言或运行时能力。
  - 文档说明每个受支持 IR 调用到 native runtime 的 ABI 约定及暂不支持范围。
...
- 当前现状：
  - 【未完成】切片尚未开始；一维数组（P0-7）已实现，切片的结果类型与存储策略仍需设计。
- 与 C17 的冲突处理原则：
