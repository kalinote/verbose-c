...
- 支持边界：
  - 当前只面向 Windows x64；支持标量、定长局部标量数组、字符串常量及值传递、标准流 `read/write`、`_exit(int)` / `exit(int)`。全局数组、数组参数/返回及地址逃逸、其他内置函数、字符串拼接、结构体、指针和 GC 对象仍明确报错。
...
- 已完成：
  ...
  - 【已完成】定长局部一维标量数组使用每元素 8 字节栈槽，保持数值类型语义、初始化和越界检查；Machine IR 与 map 验证数组来源及布局，含数组函数的栈帧和调用空间受 4096 字节预算限制。验收入口为 `tests/test_native_arrays.py` 和库存示例。
- 待完成：
  - 【未完成】全局数组、数组参数/返回及地址逃逸、对象元素数组、结构体、指针对象存储和逐对象内存回收；扩展前需要单独确定对象布局及生命周期。
