...
### 【依赖 C-P0-1】【依赖 C-P0-2】P0-3 预处理条件编译
...
### 【依赖 C-P0-4】P0-8 C 控制流补齐：`switch/case/default`
...
### 【依赖 C-P0-5】【已完成】C-P1-2 统一的类型转换与算术语义

类型检查、编译期常量求值、O1、VM 与 Windows x64 native 使用以下项目约定。定宽溢出采用明确报错策略，不依赖 C 的未定义行为。
...
| --- | --- | --- |
| `bool` | `0` / `1` | 提升为 `int` |
| `char` | 有符号 8 位，−128..127 | 提升为 `int` |
| `short` | 有符号 16 位，−32768..32767 | 提升为 `int` |
| `int` | 有符号 32 位 | 保留 `int` |
| `long`、`long long` | 有符号 64 位 | 保留类型，`long long` 的类型等级较高 |
| `float` | IEEE 754 binary32 | 每次转换和运算后舍入为 binary32 |
| `double` | IEEE 754 binary64 | 每次转换和运算后舍入为 binary64 |

- 整数字面量（包括直接带负号的字面量）选择能容纳其值的 `int`、`long`，再超出则为 VM 的 `unlimited int` 扩展。无后缀浮点字面量为 `double`；例如 `float x = 0.1` 会在初始化时转换一次。
- 一元 `+/-` 先进行整数提升。二元算术和数值比较先转换两侧为共同类型，等级为 `double > float > long long > long > int`；操作数次序不影响类型。
- 结果保留共同类型，不因结果越界自动扩宽。例如两个 `char` 相加产生 `int`，两个 `int` 相加仍是 `int`。
- 类型转换只绑定紧邻因子：`(int)1.9 * 10 == 10`，`(int)(1.9 * 10) == 19`；typedef 类型使用同一规则。

#### 转换与存储边界

- 整数之间直接转换并检查目标范围，保留超过 `2^53` 的精度。越界窄化报错，不截取低位、不回绕。
- 浮点转整数先向零截断小数，再检查目标整数范围；NaN、无穷和越界值转换失败。例如 `(char)-128.9 == -128`。
- 整数转浮点、`double` 转 `float` 使用就近偶数舍入；精度损失允许发生，结果不能溢出为无穷。允许次正规数和下溢到带符号的零。
- 整数直接转 `float` 只舍入一次；只有显式经过 `double` 的表达式才执行两次舍入，O1 不合并这些转换边界。
- 转 `bool` 时，整数零和浮点正负零均为假，其他数值为真；逻辑运算保留短路求值。
- 声明初始化、赋值、数组元素初始化、实参、返回值、显式转换均执行相同的转换规则。复合赋值和 `++/--` 先按共同类型计算，再检查并转换回左值类型。
- 常量越界转换在类型检查期报告源类型、目标类型和行号；未知值在 VM/native 实际执行转换时检查。字符串或 `void` 转数值、指针转浮点等不支持的转换在类型检查期拒绝。

#### 除法、取模与错误

- `int / int` 使用精确整数计算，商向零截断；`a % b = a - (a / b) * b`，非零余数与被除数同号。例如 `-7 / 3 == -2`，`-7 % 3 == -1`，`7 % -3 == 1`。
- 任一侧为浮点时执行浮点除法；`%` 仅接受整数或提升后的布尔值。
- 整数和浮点除零、定宽整数加减乘及取负溢出、定宽浮点溢出均明确报错。`MIN / -1` 和 `MIN % -1` 都按商溢出处理。
- O1 不能提前触发或删除可见的运行期错误。`0 && (1 / 0)` 返回 `0`；会失败的算术折叠保留原表达式。`enum` / `case` 必须在编译期求值，其数值错误立即诊断；枚举成员受 `int` 范围限制。
...
### 【依赖 C-P0-7】【依赖 C-P0-9】【依赖 C-P1-1】P1-3 `sizeof` 与更完整声明语法
...
### 【依赖 C-P0-3】【依赖 C-P0-4】【依赖 C-P1-5】【依赖 C-P1-6】【依赖 C-P1-7】P1-9 断言 `assert`（`<assert.h>`）
...
### 【依赖 C-P0-7】P2-1 多维数组与复杂初始化（C17 6.7.6 扩展）
